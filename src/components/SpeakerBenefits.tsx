...
export default function NeoBrutalismBenefits() {
  return (
    <div className="p-6 ">
      <h1 className="text-4xl font-bold mb-8 text-center bg-red-500 text-white p-4 rotate-1 inline-block">
        Why Speak at Dev-o-lution?
      </h1>
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {benefits.map((benefit, index) => (
          <Card key={index} className="p-6 bg-white/95 border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all">
...
function getBackgroundColor(index: number) {
  const colors = [
    ...
    "bg-orange-500",
    "bg-pink-500",
    "bg-teal-500",  // New color for the additional benefit
  ]
